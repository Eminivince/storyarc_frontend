...
import {
  applyTemplateToContractDraft,
  buildContractPayload,
  mapContractToDraft,
} from "../admin/adminContractUtils";
import {
  createAdminContract,
  fetchAdminContract,
  fetchAdminContractLookups,
...
export default function AdminContractEditorPage() {
  ...
  const lookupsQuery = useQuery({
    queryKey: ["admin", "contractLookups"],
    queryFn: fetchAdminContractLookups,
    staleTime: 15_000,
  });
  ...
  useEffect(() => {
    ...
    setForm(mapContractToDraft(contractQuery.data.contract));
  ...
  async function applyTemplate(templateId) {
    ...
    try {
      ...
      setForm((current) => applyTemplateToContractDraft(current, response.template));
    } catch (error) {
      showAdminNotice(
        error.message || "Could not load that template into the contract draft.",
        "info",
      );
    }
  }
  ...
  const saveMutation = useMutation({
    mutationFn: (payload) =>
      isEditing
        ? updateAdminContract(contractId, payload)
        : createAdminContract(payload),
    onError: (error) => {
      showAdminNotice(error.message || "Could not save this contract.", "info");
    },
    onSuccess: (response) => {
      showAdminNotice(response.message || "Contract saved.");
      queryClient.invalidateQueries({ queryKey: ["admin", "contracts"] });
      queryClient.invalidateQueries({ queryKey: ["admin", "contractLookups"] });
      if (contractId) {
        queryClient.invalidateQueries({ queryKey: ["admin", "contract", contractId] });
      }
      navigate(buildAdminContractDetailsHref(response.contract.id));
    },
  });
  ...
  function validateForm() {
    if (!form.userId) {
      return "Select the user tied to this contract.";
    }

    if (!form.storyId) {
      return "Select the story covered by this contract.";
    }

    if (!form.partyRole.trim()) {
    ...
    if (!form.templateName.trim()) {
      return "Template name or contract basis is required.";
    }
  ...
  function handleSave() {
    ...
    saveMutation.mutate(buildContractPayload(form));
  }

  if (
    (lookupsQuery.isLoading && !lookupsQuery.data) ||
    (contractQuery.isLoading && !contractQuery.data && isEditing)
  ) {
    return <RouteLoadingScreen />;
  }

  if (lookupsQuery.isError || contractQuery.isError) {
    return (
      <ReaderStateScreen
        ...
        description={
          ...
          "We could not load the contract editor right now."
        }
      ...
      headerActions={
        <>
          ...
          <button
            ...
            type="button"
          >
            {saveMutation.isPending ? "Saving..." : isEditing ? "Save Contract" : "Create Contract"}
          </button>
        </>
      }
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.08fr_0.92fr]">
        <div className="space-y-6">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
                ...
                <p className="mt-2 text-xl font-black">
                  {formatContractStatusLabel(form.status)}
                </p>
              </div>
              <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
                <p className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-400">
                ...
                <p className="mt-2 text-lg font-black">
                  {form.templateName || "Custom Contract"}
                </p>
                <p className="mt-1 text-xs text-slate-400">
                  {form.templateId ? "Template linked" : "Custom snapshot"}
                </p>
              </div>
              <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
                <p className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-400">
                ...
                <p className="mt-2 text-lg font-black">
                  {form.userName || "Select User"}
                </p>
                <p className="mt-1 text-xs text-slate-400">
                  {form.storyTitle || "Select Story"}
                </p>
              </div>
            </div>
          </Reveal>
          ...
          <Reveal className="rounded-[28px] border border-primary/10 bg-white p-6 shadow-[0_24px_60px_-36px_rgba(13,15,22,0.35)] dark:bg-primary/5">
            <SectionHeading icon="manage_accounts" title="Contract Setup" />
            ...
            <div className="mt-5 grid gap-5 lg:grid-cols-2">
              <label className="block">
                ...
                >
                  <option value="">Custom Contract Snapshot</option>
                  {templates.map((template) => (
                    <option key={template.id} value={template.id}>
                      {template.templateName} - {template.companyName}
                    </option>
                  ))}
                </select>
              </label>
              ...
              <label className="block">
                ...
                <input
                  className="mt-2 w-full rounded-xl border border-primary/10 bg-slate-50 px-4 py-3 text-sm focus:border-primary/30 focus:outline-none focus:ring-2 focus:ring-primary/20 dark:bg-background-dark/50"
                  onChange={(event) => updateField("partyRole", event.target.value)}
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <Reveal className="rounded-[28px] border border-primary/10 bg-white p-6 shadow-[0_24px_60px_-36px_rgba(13,15,22,0.35)] dark:bg-primary/5">
              <SectionHeading icon="gavel" title="Legal Terms" />
        ...
        <div className="space-y-6">
            ...
            <div className="mt-4 overflow-hidden rounded-[24px] border border-primary/10 bg-[#17130d]">
              <div className="flex flex-wrap items-center gap-1 border-b border-white/10 bg-white/5 p-2">
                <ToolbarButton
                  ...
                  onClick={() =>
                    showAdminNotice(
                      "Formatting controls still resolve to plain contract text in this workflow.",
                      "info",
                    )
                  }
                />
          ...
          <Reveal className="rounded-[28px] border border-primary/10 bg-white p-6 shadow-[0_24px_60px_-36px_rgba(13,15,22,0.35)] dark:bg-primary/5">
            ...
            <p className="mt-4 text-sm leading-7 text-slate-600 dark:text-slate-300">
              Selecting a template copies its terms into this contract once. Any edits you
              make here stay attached to this contract record only.
            </p>
          </Reveal>
        </div>
      </div>
